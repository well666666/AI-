...
import { Search, BookOpen, GitMerge, FileText, Sparkles, ChevronRight, ChevronLeft, X } from 'lucide-react';
...
export default function App() {
  ...
  const [searchResult, setSearchResult] = useState<{ answer: string, refs: any[], isEnglish?: boolean } | null>(null);
  const [activeCategory, setActiveCategory] = useState<'rules' | 'processes' | 'forms' | null>(null);
  const [activeFile, setActiveFile] = useState<{ id: string, title: string, content: string } | null>(null);

  const handleOpenRef = (ref: any) => {
    const categoryMap: Record<string, 'rules' | 'processes' | 'forms'> = {
      rule: 'rules',
      process: 'processes',
      form: 'forms'
    };
    const category = categoryMap[ref.type];
    if (category) {
      const file = categoryData[category].find(f => f.id === ref.id);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // 判斷是否主要為英文輸入 (包含英數字與常見標點)
      const isEnglish = /^[a-zA-Z0-9\s.,?!'"\-_]+$/.test(query.trim());
      ...
      if (bestMatch && maxScore > 0) {
        setSearchResult({ 
          answer: isEnglish ? (bestMatch as any).enAnswer : bestMatch.answer, 
          refs: bestMatch.refs,
          isEnglish
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans selection:bg-[#1A237E] selection:text-white">
      {/* Header */}
      <header className="border-b border-gray-200 bg-white/80 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-[#1A237E] flex items-center justify-center shadow-md">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            <h1 className="text-xl font-bold text-gray-900 tracking-wide">企業 AI 智慧知識庫</h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <form onSubmit={handleSearch} className="relative group">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <Search className="h-6 w-6 text-gray-400 group-focus-within:text-[#1A237E] transition-colors" />
            </div>
            <input
              type="text"
              value={query}
            ...
            <button
              ...
              className="absolute inset-y-2 right-2 px-6 bg-[#1A237E] hover:bg-[#283593] text-white rounded-xl font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm"
            >
              {isSearching ? (
                <>
          ...
          <AnimatePresence>
            {searchResult && (
              ...
              >
                <div className="bg-white border border-[#1A237E]/20 rounded-2xl p-6 shadow-lg relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-[#1A237E]"></div>
                  <div className="flex items-start gap-4">
                    <div className="mt-1 p-2 bg-blue-50 rounded-lg">
                      <Sparkles className="w-5 h-5 text-[#1A237E]" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-sm font-semibold text-[#1A237E] uppercase tracking-wider mb-2">AI 助理建議</h3>
                      <p className="text-gray-800 leading-relaxed text-lg">
                        {searchResult.answer}
                      </p>
                      
                      {/* Context References */}
                      {searchResult.refs && searchResult.refs.length > 0 && (
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3">
                          {searchResult.refs.map((ref, idx) => (
                            <div 
                              key={idx} 
                              onClick={() => handleOpenRef(ref)}
                              className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm hover:bg-blue-50 hover:border-blue-200 transition-colors cursor-pointer flex items-center justify-between group"
                            >
                              <div className="flex items-center gap-2">
                                {ref.type === 'process' && <GitMerge className="w-4 h-4 text-purple-600" />}
                                {ref.type === 'form' && <FileText className="w-4 h-4 text-green-600" />}
                                {ref.type === 'rule' && <BookOpen className="w-4 h-4 text-blue-600" />}
                                <span className="text-gray-700 font-medium">{ref.id}</span>
                              </div>
                              <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-[#1A237E] transition-colors" />
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div 
            ...
            className="group bg-white border border-gray-200 hover:border-[#1A237E] rounded-2xl p-8 transition-all hover:shadow-lg cursor-pointer relative overflow-hidden"
          >
            ...
            <div className="relative z-10">
              <div className="w-14 h-14 bg-blue-50 text-[#1A237E] rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <BookOpen className="w-7 h-7" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">規章制度</h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                查詢公司各項管理辦法、員工手冊、獎懲規定等正式規範文件。
              </p>
              <div className="flex items-center text-sm font-medium text-[#1A237E] group-hover:text-blue-800 transition-colors">
                瀏覽規章 <ChevronRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </div>
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-b from-blue-50/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <div className="relative z-10">
              <div className="w-14 h-14 bg-purple-50 text-purple-700 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <GitMerge className="w-7 h-7" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">作業流程</h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                了解各部門標準作業程序 (SOP)、跨部門協作指南與申請流程。
              </p>
              <div className="flex items-center text-sm font-medium text-purple-700 group-hover:text-purple-800 transition-colors">
                瀏覽流程 <ChevronRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </div>
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-b from-blue-50/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <div className="relative z-10">
              <div className="w-14 h-14 bg-green-50 text-green-700 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <FileText className="w-7 h-7" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">表單下載</h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-6">
                取得各類申請書、報表範本、請假單等常用空白表單檔案。
              </p>
              <div className="flex items-center text-sm font-medium text-green-700 group-hover:text-green-800 transition-colors">
                下載表單 <ChevronRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence>
          {(activeCategory || activeFile) && (
            <motion.div
              ...
              onClick={() => { setActiveCategory(null); setActiveFile(null); }}
            >
              <motion.div
                ...
                className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 flex flex-col max-h-[80vh]"
              >
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                      {!activeFile ? (
                        <>
                          {activeCategory === 'rules' && <BookOpen className="w-5 h-5 text-[#1A237E]" />}
                          {activeCategory === 'processes' && <GitMerge className="w-5 h-5 text-purple-600" />}
                          {activeCategory === 'forms' && <FileText className="w-5 h-5 text-green-600" />}
                          {activeCategory === 'rules' ? '規章制度列表' : activeCategory === 'processes' ? '作業流程列表' : '表單下載列表'}
                        </>
                      ) : (
                        <span className="truncate max-w-[250px]">{activeFile.title}</span>
                      )}
                    </h3>
                  </div>
                  <button 
                ...
                <div className="p-3 overflow-y-auto flex-1">
                  <AnimatePresence mode="wait">
                    {!activeFile ? (
                      ...
                      >
                        {activeCategory && categoryData[activeCategory].map((item, idx) => (
                          <div 
                            key={idx} 
                            onClick={() => setActiveFile(item)}
                            className="flex items-center justify-between p-4 hover:bg-blue-50 rounded-xl cursor-pointer group transition-colors"
                          >
                            <div className="flex items-center gap-4">
                              <div className="p-2 bg-gray-100 text-gray-500 rounded-lg group-hover:bg-white group-hover:text-[#1A237E] group-hover:shadow-sm transition-all">
                                {activeCategory === 'rules' && <BookOpen className="w-4 h-4" />}
                                {activeCategory === 'processes' && <GitMerge className="w-4 h-4" />}
                                {activeCategory === 'forms' && <FileText className="w-4 h-4" />}
                              </div>
                              <div>
                                <div className="text-sm font-bold text-gray-900 group-hover:text-[#1A237E] transition-colors">{item.id}</div>
                                <div className="text-sm text-gray-500 mt-0.5">{item.title}</div>
                              </div>
                            </div>
                            <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-[#1A237E] transition-colors" />
                          </div>
                        ))}
                      </motion.div>
                    ) : (
                      ...
                      >
                        <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 shadow-inner">
                          <div className="flex items-center gap-2 mb-4 pb-4 border-b border-gray-200">
                            <div className="px-2 py-1 bg-blue-100 text-[#1A237E] text-xs font-bold rounded">
                              {activeFile.id}
                            </div>
                            <div className="text-sm text-gray-500">文件預覽</div>
                          </div>
